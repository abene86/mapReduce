...
func createIntermediateFiles(jobName string, mapTaskNumber int, numberFilesToCreate int) []string {
	...
	index := 0
	for index < numberFilesToCreate {
		intermediateFileHoldsMapFunc := reduceName(jobName, mapTaskNumber, index)
		intermediateFile = append(intermediateFile, intermediateFileHoldsMapFunc)
		index++
	}
